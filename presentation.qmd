...
- Even though we have now already achieved good performance using the `lrn("classif.glmnet")` learner,
  we want to see whether we can improve our performance by using a boosting algorithm
...
-   However, this learner has many hyperparameters, some of which should be tuned!

-   Instead of specifying hyperparameters to a given value
    ```{r}
    lrn_xgb$param_set$set_values(nrounds = 10)
    ```
    we can also mark them for hyperparameter tuning

    ```{r}
    lrn_xgb$param_set$set_values(nrounds = to_tune(10, 100))
    ```
...
-   We can now train a learner on the full dataset using the optimal configuration
    ```{r}
    optimal_nrounds =
      instance$result_learner_param_vals$classif.xgboost.nrounds
    optimal_nrounds
...
-   We create a benchmark design to compare the tuned xgboost pipeline with the glmnet pipeline
    ...
    ```{r}
    design = benchmark_grid(task, list(lrn_xgb_tuned, lrn_glmnet_preproc), rsmp("cv", folds = 3))
    bmr = benchmark(design)
...
- This presentation: https://github.com/sebffischer/mlr3_UseR_berlin_2023
